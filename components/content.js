                          ...
                          {data.jobs.map((each, key)=>{
                            return (
                              <div key={key} className="">
                                  <li className="m-4 h-10 my-auto mx-auto mt-2 justify-center" id={'parent_'+key} onClick={subitem}>
                                      <span className="w-10 bg-gray-500 rounded-md uppercase pt-1 pb-1 text-center float-left text-lg text-white mr-4 lg:mr-5" id={'parent_'+key}>
                                          {each.name.slice(0,2)}
                                      </span> 
                                      <div className="md:text-base sm:text-sm justify-center" id={'parent_'+key} >{Object.keys(each.items).length} jobs for
                                      {' '+each.name}</div>  
                                  </li>
